...
- LSP auto-installation and setup ([c3_ls](https://github.com/ManuLinares/c3_ls)).
- Tree-Sitter grammar and highlights ([tree-sitter-c3](https://github.com/c3lang/tree-sitter-c3)).
- Code formatting via `:C3Format` or `:Format` (using LSP).
- Diagnostic info via `:C3Info`.
- `:C3Update [tool]`: Updates dependencies (`lsp`, `parser`, or both).
...
- `curl` and `tar` (or `unzip`) to download LSP.
...
require("c3").setup({
  lsp = {
    ...
    cmd = "c3_ls", -- We use https://github.com/ManuLinares/c3_ls but you can use whatever you want
    version = "latest", -- (1)
    compiler_path = nil, -- Custom path to c3c binary (2)
    stdlib_path = nil, -- Custom path to C3 standard library (2)
    format_on_save = false, -- Format buffer on save using LSP
    log_level = nil, -- (3) "error" | "warn" | "info" | "debug" (default: "error")
...
- *(1): Minimum version or tag name from GitHub (e.g., `"v1.2.3"`). Defaults to `"latest"`.*
- *(2): Paths are automatically resolved by locating `c3c` on your system path and querying the standard library path via `c3c compile - --build-env`. Provide absolute paths here only to explicitly override the automatic detection.*
- *(3): Minimum log severity passed to `c3_ls --log-level`.*
